<template>
  <div v-if="this.cocktail != null" class="wow fadeInUp cardd " data-wow-delay=".5s">
    ...
    <v-card class="  rounded text-center wavebg  wow fadeInUp  " data-wow-delay=".6s" min-height="200px"
      variant="shaped" max-width="200px">
      ...
      <a href="#features" @click="this.$emit('cocktailItem', this.cocktail, this.ingredientList)">
        ...
        <v-img class=" mx-auto" v-if="this.cocktail.strDrinkThumb != null" lazy-src="\assets\img\hero\images.png"
          v-bind:src="this.cocktail.strDrinkThumb" width="220px" cover 
          gradient="to bottom, rgba(0,0,0,.0),rgba(20,60,100,.2), rgba(130,150,200,.8)"></v-img>
        <v-img v-else src="\assets\img\hero\images.png" height="100%" cover></v-img>
        <!-- For responsiveness on small screen-->
        <div class="mx-auto   ">
          <v-card-title 
            class="text-dark text  mx-auto float-left float-lg-none  text-wrap w-50 d-sm-none"  center>
            {{ this.cocktail.strDrink.toUpperCase().substring(0, 18).trim() }}
          </v-card-title>
          <v-card-title 
            class="text-dark text  mx-auto float-left float-lg-none  text-wrap w-75 d-none d-sm-block"  center>
            {{ this.cocktail.strDrink.toUpperCase().substring(0, 18).trim() }}
          </v-card-title>
        </div>
